...
class StagingToBronzeETL:
    ...
    #     if test_mode:
    #         print("=== 테스트 모드로 실행 ===")
    #         self.database_name = "recipe_analytics_test" # 테스트용 DB명
    #         self.s3_warehouse_path = "s3a://reciping-user-event-logs/iceberg/test_warehouse/"
    #         # S3 데이터의 기본(base) 경로
    #         self.s3_staging_area_base = "s3a://reciping-user-event-logs/bronze/replay-user-behavior-event-logs/replay-user-events/"
    #         self.table_suffix = "_test"
    #     else:
    #         print("=== 운영 모드로 실행 ===")
    #         self.database_name = "recipe_analytics"
    ...
    #     print(f"테이블명: {self.bronze_table_simple_name}")
...
#         """
#         [수정됨] Airflow의 data_interval_start를 기반으로 S3 경로를 동적으로 결정합니다.
#         """
#         try:
#             self.create_spark_session()
#             self.create_bronze_table_if_not_exists()
...
#             # === 증분 처리 모드 (Airflow가 실행) ===
#             print(f"증분 처리 모드로 실행: {data_interval_start} ~ {data_interval_end}")
            
#             # 1. Airflow가 넘겨준 UTC 시간 문자열을 datetime 객체로 파싱
...
#             # 3. KST 시간 정보를 바탕으로 S3 경로 동적 생성
#             # S3 경로가 '.../year=YYYY/month=MM/day=DD/hour=HH/' 형태라고 가정
#             year = start_time_kst.strftime('%Y')
#             month = start_time_kst.strftime('%m')
#             day = start_time_kst.strftime('%d')
#             hour = start_time_kst.strftime('%H')
            
#             s3_path = f"{self.s3_staging_area_base}year={year}/month={month}/day={day}/hour={hour}/"
#             print(f"처리 대상 S3 동적 경로: {s3_path}")
            
#             try:
#                 raw_df = self.spark.read.text(s3_path)
#                 if raw_df.rdd.isEmpty():
#                     print(f"경로에 처리할 데이터가 없습니다: {s3_path}. 작업을 정상 종료합니다.")
#                     return
#             except Exception:
#                 print(f"경로를 찾을 수 없습니다: {s3_path}. 작업을 정상 종료합니다.")
#                 return

#             record_count = raw_df.count()
#             print(f"총 {record_count:,}건의 데이터를 처리합니다.")
            
#             bronze_df = raw_df.withColumnRenamed("value", "raw_event_string") \
#                 .withColumn("source_file", lit(s3_path)) \
#                 .withColumn("ingestion_timestamp", current_timestamp()) \
#                 .withColumn("ingestion_date", to_date(lit(start_time_kst.strftime('%Y-%m-%d'))))
            
#             print(f"Bronze Iceberg 테이블의 '{start_time_kst.strftime('%Y-%m-%d')}' 파티션에 데이터 추가(Append)...")
#             bronze_df.writeTo(self.bronze_table_simple_name).append()
#             print(f"Staging to Bronze 파이프라인 성공적으로 완료 ({record_count:,}건 처리)")
...
# def main():
#     parser = argparse.ArgumentParser(description="Stateless Staging to Bronze ETL Job")
