...
│       ├── batch_events_00.parquet     # 원시 이벤트 데이터
│       ├── batch_events_01.parquet
│       ├── ...
│       └── batch_events_27.parquet     # 총 28개 파일
│
...
**Bronze Layer (단순 파일 저장)**:
```bash
s3://reciping-user-event-logs/bronze/landing-zone/events/
├── batch_events_00.parquet  # 그냥 파일
├── batch_events_01.parquet  # 그냥 파일
└── batch_events_02.parquet  # 그냥 파일
```
→ **파일만 있음, 메타데이터 없음, 스키마 정보 없음**

**Silver Layer (Iceberg 테이블)**:
```bash
s3://reciping-user-event-logs/iceberg/warehouse/
└── recipe_analytics.db/           # 📁 데이터베이스 네임스페이스
    └── user_events_silver/        # 📊 테이블 (메타데이터 + 데이터)
        ├── metadata/              # 🔥 테이블 정의, 스키마, 파티션 정보
        │   ├── 00000-xxx.metadata.json
        │   └── snap-xxx.avro
        └── data/                  # 실제 데이터 파일들
            └── event_year=2025/event_month=7/
```
→ **테이블 = 메타데이터 + 데이터, ACID 트랜잭션, 스키마 관리**
...
*이 문서는 Reciping Data Pipeline 프로젝트의 S3 기반 데이터 레이크하우스 아키텍처를 완전히 문서화한 것입니다. 질문이나 개선 사항이 있으시면 언제든 연락 주세요.*
