...
def send_df_to_kafka_jsonl_batch(df: pd.DataFrame, topic: str, bootstrap_servers: str, batch_size: int = 1000):
    """ 
    Pandas DataFrame을 JSONL 배치 메시지로 변환하여 Kafka 토픽으로 전송합니다.
    여러 JSON 레코드를 하나의 JSONL 메시지로 묶어서 전송 (배치 방식)
    """
    conf = {'bootstrap.servers': bootstrap_servers, 'client.id': socket.gethostname()}
    producer = Producer(conf)
    
    print(f"🚀 Confluent Kafka Producer (JSONL 배치)가 브로커({bootstrap_servers})에 연결을 시도합니다.")
    print(f"   - 토픽 '{topic}'으로 {len(df):,}개의 이벤트를 JSONL 배치({batch_size}개씩)로 전송합니다...")

    records = df.to_dict('records')
    
    # 배치로 나누어서 JSONL 형태로 전송
    for i in range(0, len(records), batch_size):
        batch = records[i:i+batch_size]
        
        # JSONL 형태로 변환 (각 JSON을 줄바꿈으로 구분)
        jsonl_lines = []
        for record in batch:
            json_line = json.dumps(record, ensure_ascii=False)
            jsonl_lines.append(json_line)
        
        # JSONL 메시지 생성 (줄바꿈으로 구분된 JSON들)
        jsonl_message = '\n'.join(jsonl_lines)
        
        try:
            producer.produce(
                topic,
                value=jsonl_message.encode('utf-8'),
    ...
    if remaining > 0:
        print(f"⚠️ {remaining}개의 JSONL 배치가 아직 전송 대기 중입니다.")
    
    total_batches = (len(records) + batch_size - 1) // batch_size
    print(f"✅ {len(df):,}개의 이벤트를 {total_batches}개 JSONL 배치로 전송 요청 완료!")


def send_df_to_kafka_jsonl_individual(df: pd.DataFrame, topic: str, bootstrap_servers: str):
    """ 
    Pandas DataFrame을 개별 JSONL 메시지로 변환하여 Kafka 토픽으로 전송합니다.
    각 JSON 레코드를 개별적으로 JSONL 형식으로 전송 (기존 방식과 유사하지만 JSONL 형식)
    """
    conf = {'bootstrap.servers': bootstrap_servers, 'client.id': socket.gethostname()}
    producer = Producer(conf)
    
    print(f"🚀 Confluent Kafka Producer (개별 JSONL)가 브로커({bootstrap_servers})에 연결을 시도합니다.")
    print(f"   - 토픽 '{topic}'으로 {len(df):,}개의 이벤트를 개별 JSONL로 전송합니다...")

    records = df.to_dict('records')
    for record in records:
        # JSONL 형식: 단일 JSON 라인 (줄바꿈 없음, 하지만 JSONL 표준 준수)
        jsonl_line = json.dumps(record, ensure_ascii=False)
        
        try:
            producer.produce(
                topic,
                value=jsonl_line.encode('utf-8'),
                callback=delivery_report
            )
        except BufferError:
            producer.flush()
    ...
    print(f"✅ {len(df):,}개의 이벤트를 개별 JSONL로 전송 요청 완료!")


# 기본 함수는 배치 JSONL 방식을 사용하도록 설정
def send_df_to_kafka(df: pd.DataFrame, topic: str, bootstrap_servers: str, mode: str = 'jsonl_batch', batch_size: int = 1000):
    """ 
    Pandas DataFrame을 JSONL 형태로 Kafka 토픽에 전송하는 통합 함수
    
    Args:
        df: 전송할 DataFrame
        topic: Kafka 토픽명
        bootstrap_servers: Kafka 브로커 주소
        mode: 전송 방식 ('jsonl_batch' 또는 'jsonl_individual')
        batch_size: 배치 모드에서 사용할 배치 크기
    """
    if mode == 'jsonl_batch':
        send_df_to_kafka_jsonl_batch(df, topic, bootstrap_servers, batch_size)
    elif mode == 'jsonl_individual':
        send_df_to_kafka_jsonl_individual(df, topic, bootstrap_servers)
    else:
        print(f"⚠️ 알 수 없는 모드: {mode}. 'jsonl_batch' 또는 'jsonl_individual'을 사용하세요.")
        print(f"기본값 'jsonl_batch' 모드로 전송합니다.")
        send_df_to_kafka_jsonl_batch(df, topic, bootstrap_servers, batch_size)
...
def generate_events_by_15min_intervals(start_date, num_intervals, bootstrap_servers, topic):
    """
    [수정됨] 15분 간격으로 이벤트를 생성하고 Kafka로 직접 전송합니다. (시간 경계 포함)
    
    Args:
        start_date: 시작 날짜
        num_intervals: 생성할 15분 간격 수
        bootstrap_servers: 접속할 Kafka 브로커 주소
        topic: 데이터를 보낼 Kafka 토픽 이름
    """
    print(f"\n🕐 15분 간격 이벤트 생성 및 Kafka 전송 시작")
    ...
    for i in range(num_intervals):
        # 15분 윈도우 계산
        window_start = start_date + timedelta(minutes=i * 15)
        ...
        window_events_df = generate_events_by_time_window(
            ...
            # target_events=100_000 # 15분당 10만개
            target_events=10_000 # 15분당 1만개
        )
        
        # 2. Kafka로 JSONL 형태로 전송 (개별 메시지)
        if not window_events_df.empty:
            send_df_to_kafka(
                df=window_events_df,
                topic=topic,
                bootstrap_servers=bootstrap_servers,
                mode='jsonl_individual'  # 개별 JSONL 모드 (각 이벤트가 별도 메시지)
            )
        else:
    ...
    print(f"\n🎉 모든 15분 간격 이벤트의 JSONL Kafka 전송 요청 완료!")
    total_events = sum([w['event_count'] for w in window_results])
    print(f"📊 총 전송 요청 이벤트: {total_events:,}개")
    print(f"📝 전송 형식: JSONL 배치 (1000개씩 묶어서 전송)")
    print(f"🔗 JSONL 형태 예시:")
    print(f"   {{\"event_name\": \"view_page\", \"user_id\": \"123\", ...}}")
    print(f"   {{\"event_name\": \"click_recipe\", \"user_id\": \"456\", ...}}")
    print(f"   {{\"event_name\": \"search_recipe\", \"user_id\": \"789\", ...}}")
    
    return window_results


# ===================================================================
# 📝 JSONL Kafka 전송 방식 설명
# ===================================================================

print("\n📝 JSONL Kafka 전송 방식:")
print("=" * 50)
print("🔹 JSONL 배치 모드 ('jsonl_batch'):")
print("   - 1000개의 이벤트를 하나의 JSONL 메시지로 묶어서 전송")
print("   - 각 JSON 라인이 줄바꿈(\\n)으로 구분됨")
print("   - 네트워크 효율성 증가, 전송 오버헤드 감소")
print("   - 예시: message1\\nmessage2\\nmessage3")
print("")
print("🔹 개별 JSONL 모드 ('jsonl_individual'):")
print("   - 각 이벤트를 개별 JSONL 메시지로 전송")
print("   - 실시간성 증가, 개별 메시지 처리 용이")
print("   - 기존 JSON 방식과 유사하지만 JSONL 표준 준수")
print("")
print("🔹 사용법:")
print("   # 배치 모드 (기본값)")
print("   send_df_to_kafka(df, topic, servers, mode='jsonl_batch', batch_size=1000)")
print("   # 개별 모드")
print("   send_df_to_kafka(df, topic, servers, mode='jsonl_individual')")
print("")
print("✅ 현재 설정: JSONL 배치 모드 (1000개씩 묶어서 전송)")
...
if __name__ == "__main__":
    ...
    # 1. 커맨드 라인 인자 파서 설정
    # ==================================
    parser = argparse.ArgumentParser(description="15분 간격 이벤트 로그를 생성하여 Kafka로 전송하는 스크립트")
    
    parser.add_argument(
        '--start-date', 
        type=str, 
        required=True, 
        help="시작 시간. 형식: 'YYYY-MM-DD-HH' 또는 'YYYY-MM-DD-HH-MM'"
    )
    parser.add_argument(
        '--num-intervals', 
        type=int, 
        default=4, 
        help="생성할 15분 간격의 수. 기본값: 4 (1시간 분량)"
    )
    ...
    # ✅ 분 포함 여부 자동 감지
    try:
        if args.start_date.count('-') == 4:
            # YYYY-MM-DD-HH-MM 형식
            sim_start_datetime = datetime.strptime(args.start_date, '%Y-%m-%d-%H-%M').replace(tzinfo=KST)
        else:
            # YYYY-MM-DD-HH 형식
            sim_start_datetime = datetime.strptime(args.start_date, '%Y-%m-%d-%H').replace(tzinfo=KST)
    except ValueError:
        print("❌ 오류: 날짜 형식 오류")
        print("   허용: 'YYYY-MM-DD-HH' 또는 'YYYY-MM-DD-HH-MM'")
        exit(1)
    
    print(f"🔍 파싱된 시작 시간: {sim_start_datetime.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    
        
    sim_num_intervals = args.num_intervals

    print("=" * 50)
    print("🚀 15분 간격 이벤트 생성 및 Kafka 전송을 시작합니다.")
    print("=" * 50)
